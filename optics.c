...
trace_status trace(ray *g, const collection *scene, int N_interactions,
	...
	double r_surface_min = distance_limit;
	int i_plane = -1;
	int i_sphere = -1;
	int i_surface = -1;
	...
	for (int i = 0; i < scene->N_surfaces; i++) {
		I = intersect_ray_surface(g, &scene->surfaces[i]);
		if (I.kind != intersecting)
			continue;
		if (I.r < r_surface_min && I.r > epsilon) {
			i_surface = i;
			r_surface_min = I.r;
		}
	}

	if (i_plane == -1 && i_sphere == -1 && i_surface == -1)
		return too_far_away;
	...
	if (r_plane_min < r_sphere_min && r_plane_min < r_surface_min) {
		...
		//printf("Hit plane %d\n", i_plane+1);
	} else if(r_sphere_min < r_surface_min) {
		...
		col = scene->surfaces[i_surface].col;
		normal = scene->surfaces[i_surface].plane.normal;
		P = add(g->origin, multiply(g->direction, r_surface_min));
		mat = scene->surfaces[i_surface].mat;
		is_light_source = scene->surfaces[i_surface].is_light_source;
	}
