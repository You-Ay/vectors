...
trace_status trace(ray *g, const collection *scene, int N_interactions,
	...
	double r_circle_min = distance_limit;
	int i_plane = -1;
	int i_sphere = -1;
	int i_triangle = -1;
	...
	for(int i = 0; i < scene->N_circles; i++) {
		I = intersect_ray_circle(g, &scene->circles[i]);
		if (I.kind != intersecting)
			continue;
		if (I.r < r_circle_min && I.r > epsilon) {
			i_circle = i;
			r_circle_min = I.r;
		}
	}

	if (i_plane == -1 && i_sphere == -1 && i_triangle == -1 && i_parallelogram == -1 && i_circle == -1)
		return too_far_away;
	...
	if (r_plane_min < r_sphere_min && r_plane_min < r_triangle_min && r_plane_min < r_parallelogram_min && r_plane_min < r_circle_min) {
		...
		//printf("Hit plane %d\n", i_plane+1);
	} else if(r_sphere_min < r_triangle_min && r_sphere_min < r_parallelogram_min && r_sphere_min < r_circle_min) {
		...
		//printf("Hit sphere %d\n", i_sphere+1);
	} else if(r_triangle_min < r_parallelogram_min && r_triangle_min < r_circle_min) {
	...
	} else if (r_parallelogram_min < r_circle_min) {
		...
		col = scene->circles[i_circle].col;
		normal = scene->circles[i_circle].plane.normal;
		P = add(g->origin, multiply(g->direction, r_circle_min));
		mat = scene->circles[i_circle].mat;
		is_light_source = scene->circles[i_circle].is_light_source;
	}
